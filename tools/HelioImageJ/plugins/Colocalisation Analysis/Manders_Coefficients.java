...
public class Manders_Coefficients implements PlugIn
	 {
...
public void correlate(ImageStack img1, ImageStack img2) 
	...
	if((threshold&&ip1.getMinThreshold()>0&&ip2.getMinThreshold()>0))
		{
	...
	//IJ.showMessage("ch1: "+ ch1threshmin+"   ch2: "+ ch2threshmin);
	int scaledZ1=0;
	int scaledZ2=0;
			...
			{mask = 1;
				...
				if (mask!=0)
					...
					if (ch1+ch2!=0) 
						{N2++;	
						include=true;}
					if (((ch1>=ch1threshmin)&&(ch2>=ch2threshmin))||!threshold) include = true;
					else include = false;	
	...
	if (exclude) countP=N2;
	if (threshold) countP=Ninclude;
...
//reset prefs
	...
	Prefs.set("ICP_indexRoi.int",indexRoi);
