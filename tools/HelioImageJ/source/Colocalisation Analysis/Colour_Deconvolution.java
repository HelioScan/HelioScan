...
//
	...
	public void run(String arg) {
		...
		GenericDialog gd = new GenericDialog("Colour Deconvolution 1.2", IJ.getInstance());
		...
		//boolean doILin = gd.getNextBoolean();

		double leng, A, V, C, min, log255=Math.log(255.0);
		int i,j;
			...
			if (cosy[2]==0.0){
				if (cosz[2]==0.0){
					if ((cosx[0]*cosx[0] + cosx[1]*cosx[1])> 1){
						if (doIshow)
							IJ.log("Colour [3] has a negative R component.");
						cosx[2]=0.0;
					}
					else {
					...
					if ((cosy[0]*cosy[0] + cosy[1]*cosy[1])> 1){
						if (doIshow)
							IJ.log("Colour [3] has a negative G component.");
						cosy[2]=0.0;
					}
					else {
					...
					if ((cosz[0]*cosz[0] + cosz[1]*cosz[1])> 1){
						if (doIshow)
							IJ.log("Colour [3] has a negative B component.");
						cosz[2]=0.0;
					}
					else {
		...
		for (i=0; i<3; i++){
			...
			ip0.drawString("Colour["+(i+1)+"] R:"+(float)cosx[i]+", G:"+(float)cosy[i]+", B:"+(float)cosz[i] );
		}
		...
		// translate ------------------
                ...
                new ImagePlus(title+" (Colour[1])",outputstack[0]).show();
                new ImagePlus(title+" (Colour[2])",outputstack[1]).show();
                new ImagePlus(title+" (Colour[3])",outputstack[2]).show();
	}
	...
	void getmeanRGBODfromROI(int i, double [] rgbOD, ImagePlus imp){
		...
		IJ.showMessage("Select ROI for Colour["+(i+1)+"].\n \n(Right-click to end)");
		...
		if ((xyzf[3] & 4) == 0){// right was not pressed, but left (ROI) was
			...
			for (x=px;x<(px+w);x++){
				for(y=py;y<(py+h);y++){
					p=ip.getPixel(x,y);
					// rescale to match original paper values
					rgbOD[0]+= -((255.0*Math.log(((double)((p & 0xff0000)>>16)+1)/255.0))/log255);
					rgbOD[1]+= -((255.0*Math.log(((double)((p & 0x00ff00)>> 8)+1)/255.0))/log255);
					rgbOD[2]+= -((255.0*Math.log(((double)((p & 0x0000ff))    +1)/255.0))/log255);
				}
			}
			rgbOD[0]/=(w*h);
			rgbOD[1]/=(w*h);
