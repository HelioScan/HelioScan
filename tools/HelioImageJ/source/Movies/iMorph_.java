...
//
	...
	int newwidth = ip2.getWidth();
int newheight = ip2.getHeight();

	if (ip1.getWidth()>ip2.getWidth())
		 newwidth = ip1.getWidth();
	if (ip1.getHeight()>ip2.getHeight())
		 newheight = img1.getWidth();

	ImagePlus iMorphedPlus = NewImage.createRGBImage("iMorphed",newwidth, newheight ,nFrame,NewImage.FILL_BLACK);

int img1xshift = (newwidth - ip1.getWidth())/2; 
int img2xshift = (newwidth - ip2.getWidth())/2;
int img1yshift = (newheight - ip1.getHeight())/2;
int img2yshift =(newheight - ip2.getHeight())/2;

	iMorphedPlus.getStack().getProcessor(1).insert(ip1,img1xshift,img1yshift);
	iMorphedPlus.getStack().getProcessor(nFrame).insert(ip2,img2xshift,img2yshift);
	switch (operator) {
	case 0:
