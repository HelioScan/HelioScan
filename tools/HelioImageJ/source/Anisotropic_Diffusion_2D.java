...
* written by Jiri Janacek                                                      *
*        and Vladimir Pilny (vladimir@pilny.com)                               *
* based on CImg Library by David Tschumperle (http://cimg.sourceforge.net/)    *
* GNU GPL2 license                                                             *
* This plug-in is designed to perform Filtering on an 8-bit, 16-bit, 32-bit    *
...
{
	static final String ADTDVersion = "0.1";
	static final String ADTDDate = "2005/08/12";

	// the following are the input parameters, with default values assigned to them
	double zoom       = 1.0;   // Image magnification
	...
	static float rmerged     = 0.0f;  // ratio of original image merged to the result in each iteration
	...
	double dy0        = 0.05;  // stopping condition, when average pixel square change gets below this value, iteration is stopped
	Color label_color;
	Font font;
	int font_size = 18;
	...
	public int setup(String arg, ImagePlus imp) 
	{
		...
		this.imp = imp;
		IJ.resetEscape();
		return DOES_16+DOES_32+DOES_8G+DOES_RGB+NO_UNDO;
	} // end of 'setup' method
	...
	private boolean GUI() 
	{
		...
		gd.addNumericField("Ratio of merged original image (0..1)", rmerged, 3);

		gd.addNumericField("Stopping condition", dy0, 5);
	...
	private boolean getUserParams(GenericDialog gd) 
	{
		...
		rmerged = (float) gd.getNextNumber();
		if ((rmerged<0.0f) || (rmerged>1.0f)) rmerged = 0.0f;
		dy0 = (float) gd.getNextNumber();
	...
	public void runTD(ImageProcessor ip) 
	{
		...
		IJ.showProgress(0.0);
		if (sstats) IJ.log("\n--- "+imp.getShortTitle()+" --- a1="+a1+", a2 = "+a2+", dt = "+dt+", ratio="+rmerged);
		...
		for (int s=0; s++ < scount && !breaked; )
		{
			...
			for (int x=width; x-->0;)
				for (int y=height; y-->0;) 
				{
					...
					{
						ipf[x][y][k] = pixel[k];                        
						if (rmerged != 0) ipfo[x][y][k] = pixel[k];
					}
				}

			// get initial stats for later normalizing
			float initial_max=ipf[0][0][0], initial_min=ipf[0][0][0];
			for (int x=width; x-->0;)
				for (int y=height; y-->0;)
					for (int k=channels; k-->0;) 
					{
						if (ipf[x][y][k]>initial_max) initial_max=ipf[x][y][k];
						if (ipf[x][y][k]<initial_min) initial_min=ipf[x][y][k];
					}
			...
			ipfdif=dy0;
			for (iter=0; (iter < nb_iter) && (ipfdif >= dy0) && (!breaked); iter++)
			{
				...
				for (int x=width; x-->0;)
					for (int y=height; y-->0;) 
					{
						...
						n = Math.sqrt(u*u+v*v); u/=n; v/=n; vec1 = (float)u; vec2 = (float)v;
						...
						float f2 = (float)Math.pow(1.0f+val1+val2,-a2),
							vec11 = vec1*vec1, vec12 = vec1*vec2, vec22 = vec2*vec2;
				...
				for(int sit=0; sit < nb_smoothings && !breaked; sit++)
				{
					...
					if (dt>0) 
					{
						...
						xdt = dt/Math.max(Math.abs(max), Math.abs(min));
					} 
					else xdt = -dt;
						...
						for (int y=height; y-->0;)
							for (int k=channels; k-->0;) 
							{
								if (rmerged == 0)
								{
									...
									ipfdif += (veloc[x][y][k]*xdt)*(veloc[x][y][k]*xdt);
								}
								else 
								{
									ipfnew = rmerged*ipfo[x][y][k] + (1.0f-rmerged)*ipf[x][y][k] + veloc[x][y][k]*(float)xdt;
									ipfdif += (ipfnew-ipf[x][y][k])*(ipfnew-ipf[x][y][k]);
								}
				...
				// save result
				ipfdif = Math.sqrt(ipfdif/(width*height*channels));
				if ((scount==1 && ((save>0 && (iter+1)%save==0) || breaked)) || (iter==nb_iter-1) || (ipfdif < dy0))
				...
				if (sstats) IJ.log("\riter "+(iter+1)+", xdt = "+xdt+", diff = "+ipfdif);
