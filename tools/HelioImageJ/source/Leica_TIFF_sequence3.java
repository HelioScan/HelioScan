...
public class Leica_TIFF_sequence3 implements PlugIn {
	String dir;
	String name;
	String separator;
	int count=0;
...
public void run(String arg) 
	...
	String [] doOpen = new String [nImages];
	
	double dialogHeight=15;
	
	int nDialogs = (int)Math.round(0.5+((double)nImages/dialogHeight));

	//IJ.showMessage(""+nDialogs);
	int n=(int)dialogHeight;
	
	
	int imageN=0;

for (int d=1; d<=nDialogs;d++)
	{GenericDialog gd = new GenericDialog("Select Images to Open");
	gd.addMessage(name+" contains "+nImages+" image series");
	gd.addMessage("This is dialog " +d+" of "+nDialogs);


	if (d==nDialogs) n=(int)(nImages-((d-1)*dialogHeight));
	else n = (int)dialogHeight;
	//IJ.showMessage(""+n);
	for (int i=0; i<n; i++)
		{
		title = "";
		IJ.showStatus("Reading "+ name);
		title = findText("Series Name:", imageN);	
		title = title.substring(13,title.length()-1);
		titleArray[(int)((d-1)*dialogHeight)+i]=title;
	...
	int start= (int)((d-1)*dialogHeight);
	for(int j=0; j<n; j++)
		{if (gd.getNextBoolean()) doOpen[start+j]="1";
		else doOpen[start+j]="0";
		}
	}
	split = true;
	...
	for (int i=0; i<nImages; i++)
		{
		//IJ.write(titleArray[i]);
		if (doOpen[i]=="1")
			{
			IJ.showStatus("Reading "+ name);
			
			strXcal = findText("Voxel-Width",i);		
			if (strXcal!="") strUnits = strXcal.substring(13, strXcal.length()-13+1);
			xCal= getDouble(findText("Voxel-Width",i), 18, 0);
			...
			if(mode!="")
				...
				if (mode.indexOf("z")>0)
					...
					tCal =  getDouble(findText("Delay[ms]",i), 18, 0);
