...
public class Leica_TIFF_sequence4 implements PlugIn {
	String dir;
	String name;
	String separator;
	int count=0;
...
public void run(String arg) 
	...
	skipValues = getSkipValues(nImages);

	IJ.showMessage(""+nImages);
...
public int [] getSkipValues(int imageNumber)
	{int [] skipValues = new int [imageNumber];
...
//skip to image j
		while ((j<imageNumber||line!=null))
			{n++;
			
			if (line.indexOf(" NEXT IMAGE")>0) {skipValues[j]=n; j++;}
			line = r.readLine();
			}
		r.close();
		for (int i=0; i<imageNumber; i++) IJ.write(""+n);
		return(skipValues);
		}	
...
public String findText(String text, int imageNumber)
	...
	try
		...
		int j=0;
//find the series
	if (imageNumber!=0) r.skip(skipValues[imageNumber]);
